...
Surface sdFirefly(vec3 p, vec3 offset, float r, vec3 col, float glowStrength) {
    ...
    // Add glowing effect
    vec3 glow = vec3(1.0, 0.8, 0.4) * glowStrength / (0.01 + d * d);
    col = mix(col, glow, clamp(1.0 - d / r, 0.0, 1.0)); // Smooth transition from glow to surface color
...
Surface sdTree(vec3 p, vec3 position) { 
    ...
    vec3 pyramidPos1 = vec3(position.x, position.y - 0.75, position.z);
    Surface pyramid1 = sdPyramid(p - pyramidPos1, 6., vec3(0.188, 0.502, 0.114));
    
    vec3 pyramidPos2 = vec3(position.x, position.y + 1., position.z);
    Surface pyramid2 = sdPyramid(p - pyramidPos2, 4., vec3(0.188, 0.502, 0.114));
    
    Surface co = opSmoothUnion(pyramid1, cylinder, .8); 
    co = opSmoothUnion(co, pyramid2, .4);
...
Surface sdScene(vec3 p) {
    float spacing = 5.0; // Distance between trees
    vec3 floorColor = vec3(0.161, 0.388, 0.106); // Dark green
    Surface co = sdFloor(p, floorColor);

    int seed = 567;

    // Generate trees on a grid with random offsets
    for (int x = -3; x <= 3; x++) { // Adjust range for more trees
        for (int z = -3; z <= 3; z++) {
            float randomX = (nextFloat(seed) * 1.4) * 2.0; // Random offset [-1, 1]
            float randomZ = (nextFloat(seed) * 2.3) * 2.0; // Random offset [-1, 1]
    ...
    // Fireflies for added atmosphere
    for (int i = -10; i < 10; i++) {
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  vec3 col = vec3(0);
  vec3 lp = vec3(0, 0.5, 2.); // lookat point (aka camera target)
  vec3 ro = vec3(3, 4, -1); // ray origin that represents camera position
